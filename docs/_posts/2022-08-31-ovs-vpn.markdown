...
For example, suppose there are two VMs: one for private network, and another for public network.
Many enterprises and organizations deploy their private network for privacy.
They manage their private information which not supposed to leak.
